...
cache_config_cmd_map = {
    "ycsb": {
        "0.4": "python modify_config.py server_data_len=2057080320 segment_size=8016640 block_size=256",
        "0.2": "python modify_config.py server_data_len=1028540160 segment_size=8016640 block_size=256",
        "0.1": "python modify_config.py server_data_len=514270080 segment_size=4008320 block_size=256"
    }
    "webmail-all": {
...
def get_make_cmd(build_dir, method, wl, size, running_opts=running_default_opt):
    ...
    return f"cd {build_dir} && {cmake_cmd} -DCMAKE_EXE_LINKER_FLAGS=\"-lpthread -lcityhash\" && make clean && make -j 16"
