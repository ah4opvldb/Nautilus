...
class CMDManager(object):
    def __init__(self, cluster_ips: list, port: int = 2345):
        self._cluster_ips = cluster_ips
        self._conn_list = [Connection(ip, port=port) for ip in self._cluster_ips]

    def execute_all(self, cmd):
        prom_list = []
        for c in self._conn_list:
            prom = c.run(f'{cmd}', asynchronous=True)
    ...
    def execute_on_node(self, node_id, cmd, asynchronous=True):
        return self._conn_list[node_id].run(f'{cmd}', asynchronous=asynchronous)
