...
#define RESIZE_THREAD_NUM 4
#define EXPAND_START_TIME 200000000 //resize start
#define SHRINK_START_TIME 1540000000 //resize start
...
void* client_ycsb(void* _args) {
  ...
  printf("load finished!, success_rate: %f\n", (load_success_num * 1.0) / load_num);
  ...
  // warmup for 20 seconds
  gettimeofday(&st, NULL);
  uint32_t warmup_seq = 0;
  uint64_t warmup_success_num = 0;
  while (true) {
    uint32_t idx = warmup_seq % trans_wl.num_ops;
    uint64_t key_addr, val_addr;
    uint32_t key_size, val_size;
    uint8_t op;
    get_workload_kv(&trans_wl, idx, &key_addr, &val_addr, &key_size, &val_size, &op);
    if (op == GET) {
      ret = client->kv_get((void*)key_addr, key_size, tmp_buf, &tmp_len);
      if (ret == -1) {
        ret = client->kv_set((void*)key_addr, key_size, (void*)val_addr,
  ...
  printf("client %d warmup %d ops, set_key_range: %d, get_key_range: %d, total_key_range: %d, success_rate: %f\n", args->cid, warmup_seq, 
          client->set_key_range.size(), client->get_key_range.size(), client->total_key_range.size(), (warmup_success_num * 1.0) / warmup_seq);
  ...
  uint32_t seq = 0;
  uint32_t n_miss = 0;
  uint32_t n_localcache_hit = 0;
  uint32_t n_from_remote = 0;
  uint32_t n_get = 0;
  uint32_t n_set = 0;
  while (true) {
    uint32_t idx = seq % trans_wl.num_ops;
...
// #endif
  ...
  printf("Client %d finish %d ops with %f miss ratio, with local_cache hit: %f, remote_cache hit: %f, total_key_range: %d\n", 
        args->cid, seq, (float)n_miss / n_get, (float)n_localcache_hit / n_get, 
        (float)n_from_remote / n_get, client->total_key_range.size());

  json trans_res;
  trans_res["ops"] = seq/args->run_tims_s;
...
#ifdef scaling_test
std::atomic<int> expand_signal(0);
std::atomic<int> shrink_signal(0);
void* client_ycsb(void* _args) {
  ClientArgs* args = (ClientArgs*)_args;
  DMCClient* client = args->client;
  DMCMemcachedClient con_client(args->controller_ip);
  ...
  // ready to run workload
  struct timeval st, tst, tet, trt, internal_start, internal_end;
  char tmp_buf[128];
  uint32_t tmp_len;
  std::map<uint32_t, uint32_t> lat_map;
  ...
  // sync to load ycsb dataset————————————loading——————————————
  uint64_t load_num = 0;
  uint64_t load_success_num = 0;
  lat_map.clear();
  printd(L_INFO, "client %d waiting sync", args->cid);
...
// ——————————————————————————————————test——————————————————————————————————————————
  ...
  uint64_t rolling_iter = 0;
  uint32_t internal_miss = 0;
  uint32_t internal_get = 0;
  uint32_t internal_cache_hit = 0;

  gettimeofday(&internal_start, NULL);
  while (true) {
    gettimeofday(&trt, NULL);
    //resize client
    if (rolling_iter >= 256 * 4096) {
      gettimeofday(&internal_end, NULL);
      double seconds = diff_ts_us(&internal_end, &internal_start) / 1000000.0;
      double now_time = diff_ts_us(&internal_end, &st) / 1000000.0;
      printf("%.2f %.2f\n", now_time+1500, 256 * 4096 / seconds);
      rolling_iter = 0;
      internal_start = internal_end;
      internal_get = 0;
      internal_miss = 0;
      internal_cache_hit = 0;
    }
    if(diff_ts_us(&trt, &st) > EXPAND_START_TIME && 
        client->get_sid() <= RESIZE_THREAD_NUM && client->local_stage_ == 0){
      // start expanding
      gettimeofday(&tst, NULL);
      ret = client->resize_hash_table(client->get_sid(), RESIZE_THREAD_NUM, client->get_bucket_num() * 2);
      client->local_stage_ = 1;
      expand_signal.fetch_add(1); // can be atomic
      gettimeofday(&tet, NULL);
      printf("Client %d resize end!   resize time :%ds\n", client->get_sid(), diff_ts_us(&tet, &tst) / 1000000.0);
      lat_map[diff_ts_us(&tet, &tst)]++;
    }

    if(diff_ts_us(&trt, &st) > SHRINK_START_TIME && 
        client->get_sid() <= RESIZE_THREAD_NUM && client->local_stage_ == 1){
      // start shrinking
      ret = client->resize_hash_table(client->get_sid(), RESIZE_THREAD_NUM, client->get_bucket_num() / 2);
      client->local_stage_ = 2;
      shrink_signal.fetch_add(1); // can be atomic
      gettimeofday(&tet, NULL);
      lat_map[diff_ts_us(&tet, &tst)]++;
    }
    if (expand_signal.load() == RESIZE_THREAD_NUM && client->global_stage_ == 0){ 
      client->set_bucket_num(client->get_bucket_num() * 2);
      gettimeofday(&tet, NULL);
      printf("Client %d resize end!   time :%.2fs\n", client->get_sid(), diff_ts_us(&tet, &st)/1000000.0);
      client->global_stage_ = 1;
    }
    if (shrink_signal.load() == RESIZE_THREAD_NUM && client->global_stage_ == 1){ 
      client->set_bucket_num(client->get_bucket_num() / 2);
      printf("Client %d resize end!   time :%.2fs\n", client->get_sid(), diff_ts_us(&tet, &st)/1000000.0);
      client->global_stage_ = 2;
    }
    uint32_t idx = seq % trans_wl.num_ops;
    uint64_t key_addr, val_addr;
    uint32_t key_size, val_size;
...
// #endif
      gettimeofday(&tst, NULL);
      // can't set while resizing
      if(diff_ts_us(&tst, &st) < EXPAND_START_TIME || 
          (client->global_stage_ == 1 && diff_ts_us(&tst, &st) < SHRINK_START_TIME) || 
          client->global_stage_ == 2)
        ret = client->kv_set((void*)key_addr, key_size, (void*)val_addr,
                             val_size); // 模拟从SSD中把数据调到缓存中
      } else if (ret == 1) {
...
// #endif
  ...
  // printf("Client %d finished %d ops with %f miss ratio\n", args->cid, seq,
  //        (float)n_miss / seq);
  printf("Client %d finished %d ops with %f miss ratio, with local_cache hit: %f, from remote: %f, set: %f, total_key_range: %d\n", 
          args->cid, seq, (float)n_miss / seq, (float)n_localcache_hit / seq, (float)n_from_remote / seq, (float)n_set / seq, client->total_key_range.size());
  return NULL;
}
...
void run_client(const InitArgs* args) {
  ...
  // 创建一个WTinyLFUCache数组，数组内有16个对象
  WTinyLFUCache CiCS[16] = {
      WTinyLFUCache(62, 5625, 625),
      WTinyLFUCache(62, 5625, 625),
      WTinyLFUCache(62, 5625, 625),
      WTinyLFUCache(62, 5625, 625),
      WTinyLFUCache(62, 5625, 625),
  ...
  for (int i = 0; i < num_clients; i++) {
    ret = load_config(args->config_fname, &client_conf_list[i]); // client属性初始化
    assert(ret == 0);
    ...
    for (int i = 0; i < 16; i++)
      client_main_arg_list[i].CiCS[i] = (WTinyLFUCache *)&(CiCS[i]);
