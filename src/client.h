...
class DMCClient {
 ...
 public:
  ...
  std::unordered_map<uint64_t, bool> set_key_range;
  std::unordered_map<uint64_t, bool> get_key_range;
  std::unordered_map<uint64_t, bool> total_key_range;
 ...
 private:
  ...
  void kv_set_read_index(KVOpsCtx* ctx);
  void kv_set_update_index(KVOpsCtx* ctx);
  void kv_set_insert_index(KVOpsCtx* ctx);
  void kv_set_write_block(KVOpsCtx* ctx);
 ...
 public:
  ...
  // resize
  int resize_hash_table(uint16_t client_id, uint16_t client_num, uint64_t bucket_target_num);
  int expand_hash_table(uint16_t client_id, uint16_t client_num);
  int shrink_hash_table(uint16_t client_id, uint16_t client_num);
  int get_bucket_num(){
    return bucket_num_;
  }
  int get_sid(){
  ...
  void set_bucket_num(uint64_t num){
    bucket_num_ = num;
  }
};
