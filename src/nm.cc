...
int UDPNetworkManager::rdma_cas_sid_sync(uint16_t server,
  ...
  if (wc.status != IBV_WC_SUCCESS) {
    printd(L_ERROR, "WC status(%d) wrid(%ld)", wc.status, wc.wr_id);
    printf("remote_addr: %ld, local_addr:%ld, WC status(%d) wrid(%ld), expect_val: %ld, swap_val: %ld\n", 
            remote_addr, local_addr, wc.status, wc.wr_id, expect_val, swap_val);
  }
  assert(wc.status == IBV_WC_SUCCESS);
...
int UDPNetworkManager::rdma_read_sid_sync(uint16_t server,
  ...
  if (wc.status != IBV_WC_SUCCESS) {
    printd(L_ERROR, "WC status(%d) wrid(%ld)", wc.status, wc.wr_id);
    printf("remote_addr: %ld, local_addr:%ld, len:%d, WC status(%d) wrid(%ld)\n", remote_addr, local_addr, len, wc.status, wc.wr_id);
  }
  assert(wc.status == IBV_WC_SUCCESS);
  assert(wc.wr_id == 100);
  assert(wc.opcode == IBV_WC_RDMA_READ);
...
int UDPNetworkManager::rdma_cas_and_read_sid_sync(uint16_t server,
                                          uint64_t remote_addr,
                                          uint32_t rkey,
                                          uint64_t local_addr,
                                          uint32_t lkey,
                                          uint32_t len,
                                          uint64_t local_cas_addr,
                                          uint64_t expect_val,
                                          uint64_t swap_val) {
  struct ibv_send_wr* head_wr = NULL;
  struct ibv_send_wr cas_sr, read_sr;
  struct ibv_send_wr* bad_wr;
  struct ibv_sge cas_sge, read_sge;

  ib_create_sge(local_addr, lkey, len, &read_sge);
  ...
  read_sr.wr.rdma.remote_addr = remote_addr;
  read_sr.wr.rdma.rkey = rkey;
  head_wr = &read_sr;

  ib_create_sge(local_cas_addr, lkey, 8, &cas_sge);

  memset(&cas_sr, 0, sizeof(struct ibv_send_wr));
  cas_sr.wr_id = 121321;
  cas_sr.next = head_wr;
  cas_sr.sg_list = &cas_sge;
  cas_sr.num_sge = 1;
  cas_sr.opcode = IBV_WR_ATOMIC_CMP_AND_SWP;
  cas_sr.send_flags = 0;
  cas_sr.wr.atomic.compare_add = expect_val;
  cas_sr.wr.atomic.swap = swap_val;
  cas_sr.wr.atomic.remote_addr = remote_addr;
  cas_sr.wr.atomic.rkey = rkey;
  head_wr = &cas_sr;

  struct ibv_qp* qp = rc_qp_list_[server];
  int ret = ibv_post_send(qp, &cas_sr, &bad_wr);
  assert(ret == 0);
...
int UDPNetworkManager::rdma_post_send_sid_sync_two(struct ibv_send_wr* sr,
                                               uint16_t server) {
  struct ibv_qp* qp = rc_qp_list_[server];
  struct ibv_send_wr* bad_wr;
  int ret = ibv_post_send(qp, sr, &bad_wr);
