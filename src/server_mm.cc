...
ServerMM::ServerMM(const DMCConfig* conf, struct ibv_pd* pd) {
  ...
  uint32_t list_size = get_list_size(HASH_NUM_BUCKETS * HASH_BUCKET_ASSOC_NUM); // 16*8*17170432=2197815296
  ...
  printf("cache size: %ldMB, index size: %ldMB, list size: %ldMB\n", free_space_len_/1024/1024, index_area_len_/1024/1024, list_size/1024/1024);
  ...
  printf("num of segment:%ld, base_len_:%llu\n", num_segments_, base_len_);
