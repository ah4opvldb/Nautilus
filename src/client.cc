...
DMCClient::DMCClient(const DMCConfig* conf) {
  ...
  // mycache = new WTinyLFUCache(1000, 90000, 10000);
...
void DMCClient::get_slot_raddr(const KVOpsCtx* ctx,
  ...
  bucket_raddr = ctx->init_bucket_raddr; // 这两个右值已经在create_op_ctx()中赋值
  bucket_sid = ctx->init_bucket_sid; // 这两个右值已经在create_op_ctx()中赋值
...
int DMCClient::alloc_segment(KVOpsCtx* ctx) {
  ...
  *(uint16_t*)((uint64_t)ctx->op_laddr + sizeof(uint8_t)) = my_sid_;
  ...
  uint64_t alloc_addr =
      *(uint64_t*)(ctx->op_laddr + block_size_ + sizeof(uint8_t)); // 将起始地址取出来，其中block_size_的内存块用于发送请求，后面有一个字节存储recv请求收到的请求类型，然后后面是新segment的基地址
  if (alloc_addr == 0) {
    printd(L_INFO, "Server out-of-memory");
    server_oom_ = true;
    return -1;
  }
  mm_->add_segment(alloc_addr, server_rkey_map_[0], 0); // 不支持多个server，这个函数将新获取的segment先加入空闲链表，然后再将其切片成block，再加入空闲block链表
  to_alloc_sid++;
  return 0;
#endif
}
...
// read history header (64bit) and read bucket and write data_item
void DMCClient::kv_set_read_index(KVOpsCtx* ctx) {
  printd(L_DEBUG, "kv_set_read_index");
  int ret = 0;
...
// find target cache item, and initiate key_found/target_slot_laddr/target_slot_raddr/target_block_raddr/target_block_laddr in ctx
void DMCClient::read_and_find_kv(KVOpsCtx* ctx) {
  ...
  for (int i = 0; i < ctx->num_fp_match; i++) {
    ...
    if (is_key_match(key, key_len, ctx->key, ctx->key_size)) {
      ...
      (*(ObjHeader*)local_buf_).version = header->version + 1;
      (*(ObjHeader*)local_buf_).lock = 0;
      break;
    }
  }
}
...
retry_update:
  ...
  assert(ctx->target_block_raddr % 8 == 0);
  // nm_->rdma_cas_sid_sync(0, ctx->target_block_raddr, server_rkey_map_[0],
  //                        ctx->op_laddr, local_buf_mr_->lkey, 0, 1);
  // nm_->rdma_read_sid_sync(0, ctx->target_block_raddr, server_rkey_map_[0], ctx->read_buf_laddr, local_buf_mr_->lkey,
  //                         block_size_); // todo:合并cas和read
  nm_->rdma_cas_and_read_sid_sync(0, ctx->target_block_raddr, server_rkey_map_[0], ctx->read_buf_laddr, local_buf_mr_->lkey,
                          block_size_, ctx->op_laddr, 0, 1);
  ...
  if (*(uint64_t*)ctx->op_laddr != 0) {
    if (*(uint64_t*)ctx->op_laddr != 1) {
      // error
      ctx->ret = DMC_SET_RETURN;
      assert(false);
      return;
  ...
  (*(ObjHeader*)local_buf_).version = header->version + 1; // update version
  ...
  write_sr.wr.rdma.remote_addr = ctx->target_block_raddr;
  write_sr.wr.rdma.rkey = server_rkey_map_[0]; // not ctx->kv_remote_block.rkey
  head_wr = &write_sr;
  num_rdma_write_++;
  ...
  printd(L_DEBUG, "Writing (k_size: %d v_size: %d) to %d:0x%lx",
         ...
         ctx->kv_remote_block.server, ctx->target_block_raddr);
...
void DMCClient::kv_set_insert_index(KVOpsCtx* ctx) {
  printd(L_DEBUG, "kv_set_insert_index");
...
retry_insert:
  ...
  retry_times ++;
  if (retry_times > 5) {
    printf("???: %ld, slot:%ld\n", *(uint64_t*)ctx->op_laddr, *(uint64_t*)slot);
    assert(false);
  ...
  if (*(uint64_t*)ctx->op_laddr != *(uint64_t*)slot) {
    if (ctx->key_found == false) {
      ...
      kv_set_read_index(ctx);
      match_fp_and_find_empty(ctx);
      if (ctx->num_fp_match > 0) {
        read_and_find_kv(ctx); // (2.1) read the potential kv, and check the Key
      }
      if (ctx->key_found == true) {
        ...
        find_empty(ctx);
        // update slot
        slot = (Slot*)(ctx->target_slot_laddr);
        goto retry_insert;
      }
      else {
        mm_->free(&ctx->kv_remote_block);
  ...
  // key_found==false，插入操作，那么new_slot中pointer已经在本函数早期赋值，slotmeta为0。target_slot_laddr指向bucket_laddr再加上目标slot对应偏移的地址
  // key_found==true，对于更新请求，在read_and_find_kv时已经将旧的slot写进去了，由于是就地更新，那么不需要更改指针，只需要在update_priority更新slotmeta即可
  memcpy((void*)ctx->target_slot_laddr, &ctx->new_slot, sizeof(Slot));
}

void DMCClient::kv_set_write_block(KVOpsCtx* ctx) {
  printd(L_DEBUG, "kv_set_write_block");
...
void DMCClient::update_priority_sample_adaptive(KVOpsCtx* ctx) {
  ...
  if (ctx->key_found != true) // 插入操作，需要初始化meta为0
...
int DMCClient::kv_set_1s(void* key,
  ...
  KVOpsCtx ctx;
  int ret = 0;
  create_op_ctx(&ctx, key, key_size, val, val_size, SET); // set data item in local_buf_

  int cache_idx = ctx.key_hash % 16;
  WTinyLFUCache* mycache = CiCS[cache_idx];
  
  uint64_t mykey = ctx.key_hash;
  CacheItem cached_item;
  bool get_from_cache = mycache->get(mykey, cached_item);


  set_key_range[mykey] = true;
  total_key_range[mykey] = true;
  if (get_from_cache) {

    // initiate
    ctx.key_found = true;
    ctx.target_slot_laddr = cached_item.cached_idx * sizeof(Slot) + ctx.bucket_laddr;
    get_slot_raddr(&ctx, ctx.target_slot_laddr, &ctx.target_slot_raddr, &ctx.target_slot_sid);
    // ctx.target_block_laddr = ctx.read_buf_laddr;                 
    ctx.target_block_raddr = cached_item.cached_pointer;


    kv_set_update_index(&ctx);

    if (ctx.ret == DMC_SET_RETURN) {
      mycache->remove(mykey);
      get_from_cache = false;
      goto insert_to_remote;
    }
...
insert_to_remote:
    // find target cache item, and (1) cas, (2) write
    kv_set_read_index(&ctx);
    match_fp_and_find_empty(&ctx);
    if (ctx.num_fp_match > 0) {
      read_and_find_kv(&ctx); // (2.1) read the potential kv, and check the Key
    ...
    if (ctx.key_found == true) {
      kv_set_update_index(&ctx); // (2.2) if the Key matches, update: cas,write
    } else if (ctx.num_free_slot > 0) { // insert
      kv_set_alloc_rblock(&ctx);
      find_empty(&ctx);
      kv_set_insert_index(&ctx); // (2.2) or fill in an empty slot
    } else { // evict on slot
      kv_set_alloc_rblock(&ctx);
      // key not found and there is no empty slot
      // kv_set_write_block(&ctx);
      do {
        ret = evict_bucket(&ctx); // (2.2) or evict one slot and fill in
      } while (ret == -1);
      if (ret == 1) {
        match_fp_and_find_empty(&ctx);
        find_empty(&ctx);
        kv_set_insert_index(&ctx);
      }
    }
  ...
  assert(ctx.target_slot_laddr != 0);
  update_priority(&ctx); // async WRITE+FAA

  if (ctx.key_found == false) { // duplicate kv
    ctx.num_fp_match = 0;
...
int DMCClient::kv_get_1s(void* key,
  ...
  if(get_from_cache) {
    ...
    if (cached_item.from == 1 || cached_item.from == 2) {
      nm_->rdma_read_sid_sync(0, ctx.target_block_raddr, server_rkey_map_[0], ctx.read_buf_laddr, local_buf_mr_->lkey,
                              block_size_);
      // check key
      ObjHeader* header = (ObjHeader*)ctx.read_buf_laddr;
      ...
      if (!is_key_match(key, key_len, ctx.key, ctx.key_size)) {
        mycache->remove(mykey);
        get_from_cache = false;
        goto get_from_remote;
      }

      // 如果是来自2，那么需要将读到的value写入cache中LRU3对应的缓存项
      if (cached_item.from == 2) {
        // 需要补充这个逻辑，应该先读取kv，然后构建新的cache_item，然后插入到LRU3中
      }
    }
    else {
      nm_->rdma_read_sid_sync(0, ctx.target_block_raddr, server_rkey_map_[0], ctx.read_buf_laddr, local_buf_mr_->lkey,
                              sizeof(ObjHeader));
      ObjHeader* header = (ObjHeader*)ctx.read_buf_laddr;
      if (cached_item.cached_version != header->version) {
        nm_->rdma_read_sid_sync(0, ctx.target_block_raddr, server_rkey_map_[0], ctx.read_buf_laddr, local_buf_mr_->lkey,
                                block_size_);
        mycache->remove(mykey);
        get_from_cache = false;
...
get_from_remote:
    kv_get_read_index(&ctx); // READ+READ
    match_fp_and_find_empty(&ctx);
    if (ctx.num_fp_match == 0) {
      printd(L_DEBUG, "No match fp found");
      return -1;  // not found
    } else {
      read_and_find_kv(&ctx); // 依次寻找fp相同的数据项并发起RDMA READ，基本上都是只有一个候选人
      if (ctx.key_found == false) {
  ...
  if (!get_from_cache) {
    CacheItem new_cached_item;
    // initiate new_cached_item
    new_cached_item.key = mykey;
    new_cached_item.cached_version = (*(ObjHeader*)local_buf_).version-1;
    new_cached_item.cached_idx = (ctx.target_slot_laddr - ctx.bucket_laddr) / sizeof(Slot);
    new_cached_item.cached_pointer = ctx.target_block_raddr; // 注意放入缓存时地址48-64位的转换

    mycache->put(new_cached_item);
    set_key_range[mykey] = true;
    total_key_range[mykey] = true;
  }

  assert(mykey == ctx.key_hash);

  return get_from_cache ? 1 : 0;
}
...
int DMCClient::resize_hash_table(uint16_t client_id, uint16_t client_num, uint64_t bucket_target_num){
  
  if(bucket_target_num > bucket_num_){ //expand
    int n = bucket_target_num / bucket_num_;
    if(bucket_target_num % bucket_num_ == 0 && ((n & (n - 1)) == 0)){
      while ((n & 1) == 0){
        int ret = expand_hash_table(client_id, client_num);
        if(ret) return ret;
        n >>= 1;
      }
      return 0;
  ...
  else if(bucket_target_num == bucket_num_){ //no need to resize
    return 0;
  }
  else {
    int n = bucket_num_ / bucket_target_num;
    if(bucket_num_ % bucket_target_num == 0 && (n & (n - 1)) == 0){
      while ((n & 1) == 0){
        int ret = shrink_hash_table(client_id, client_num);
        if(ret) return ret;
        n >>= 1;
      }
      return 0;
...
//扩展两倍
int DMCClient::expand_hash_table(uint16_t client_id, uint16_t client_num){
  printf("Client %d resize begin!\n", client_id);
  //目标：将原来块内的部分槽转移到新的bucket内
  uint32_t st_bucket_id = (bucket_num_ / client_num) * (client_id - 1);
  uint32_t end_bucket_id = st_bucket_id + (bucket_num_ / client_num);
  uint64_t r_addr, rw_addr;
  uint32_t rkey = server_rkey_map_[0];
  uint32_t lkey = local_buf_mr_->lkey;
  for(int i = st_bucket_id; i < end_bucket_id; i += 4){ //遍历线程范围内的所有bucket
    r_addr = i * sizeof(Bucket) + server_base_addr_;
    nm_->rdma_read_sid_sync(0, r_addr, rkey, (uint64_t)local_buf_, lkey,
                            sizeof(Bucket) * 4);
    for(int j = 0; j < 4; j++){ //遍历一次读取的4个bucket
      Slot *slots = (Slot *)(local_buf_ + j * sizeof(Bucket));
      int move_num = 0; //该bucket需要挪动的slot数
      Slot *local_slots = (Slot *)(local_buf_ + 4 * sizeof(Bucket)); //构造新的bucket
      
      //在线程内存地址开一段空间，存修改后的bucket，以及新的bucket内的slot数组
      for(int k = 0; k < HASH_BUCKET_ASSOC_NUM; k++){ //遍历一个bucket内的所有slot
        if((i + j) != (slots[k].meta.acc_info.key_hash % (bucket_num_ * 2))){
          memcpy(&local_slots[move_num], &slots[k], sizeof(Slot)); //迁移
          move_num ++;
          slots[k].atomic.kv_len = 0; //清空原来的Slot
        }
      }
      if(move_num == 0) continue;
      rw_addr = (i + j + bucket_num_) * sizeof(Bucket) + server_base_addr_;
      nm_->rdma_write_sid_sync(0, rw_addr, rkey, (uint64_t)local_slots, lkey, 
                                    sizeof(Slot) * move_num);
    }
    rw_addr = i * sizeof(Bucket) + server_base_addr_;
    nm_->rdma_write_sid_sync(0, r_addr, rkey, (uint64_t)local_buf_, lkey,
                            sizeof(Bucket) * 4); //覆盖掉旧的bucket
  }
  return 0;
}

//缩小为原来一半
int DMCClient::shrink_hash_table(uint16_t client_id, uint16_t client_num){
  printf("Client %d resize begin!\n", client_id);
  //目标：将右边的区域内数据转移到左边区域的bucket内
  uint32_t zone_size = (bucket_num_ / 2) / client_num;
  uint32_t st_lbucket_id = zone_size * (client_id - 1);
  uint32_t end_lbucket_id = st_lbucket_id + zone_size;
  uint32_t st_rbucket_id = zone_size * (client_id - 1) + bucket_num_ / 2;
  uint32_t end_rbucket_id = st_rbucket_id + zone_size;

  uint64_t r_raddr, r_laddr, rw_addr;
  uint32_t rkey = server_rkey_map_[0];
  uint32_t lkey = local_buf_mr_->lkey;

  for(int i = st_lbucket_id; i < st_rbucket_id; i += 4){
    r_laddr = i * sizeof(Bucket) + server_base_addr_;
    // 读取左桶和右桶
    nm_->rdma_read_sid_sync(0, r_laddr, rkey, (uint64_t)local_buf_, lkey,
                            sizeof(Bucket) * 4);
    Bucket * lbuckets = (Bucket *)local_buf_;
    r_raddr = (i + bucket_num_ / 2) * sizeof(Bucket) + server_base_addr_;
    nm_->rdma_read_sid_sync(0, r_raddr, rkey, (uint64_t)local_buf_ + sizeof(Bucket) * 4, lkey,
                            sizeof(Bucket) * 4);
    Bucket * rbuckets = (Bucket *)local_buf_ + sizeof(Bucket) * 4;
    for(int j = 0; j < 4; j ++){
      int lp, rp; //遍历左右两个bucket
      Slot *rslots = rbuckets[j];
      Slot *lslots = lbuckets[j];
      std::priority_queue<Slot *, std::vector<Slot*>, CompareSlot> rslot_queue;
      for(rp = 0; rp < HASH_BUCKET_ASSOC_NUM; rp++){
        uint64_t kv_raddr = HashIndexConvert48To64Bits(rslots[rp].atomic.pointer);
        if(kv_raddr != 0){
          rslot_queue.push(&rslots[rp]); //将不为空的slot插入优先队列
        }
      }
      if(rslot_queue.empty()) continue;
      for(lp = 0; lp < HASH_BUCKET_ASSOC_NUM; lp++){
        uint64_t kv_raddr = HashIndexConvert48To64Bits(lslots[lp].atomic.pointer);
        if(kv_raddr == 0 && !rslot_queue.empty()){ //左边bucket的空槽
          Slot *temp = rslot_queue.top();
          rslot_queue.pop();
          memcpy(&lslots[lp], temp, sizeof(Slot)); //数据迁移
          // temp->atomic.kv_len = 0; //清空原来的Slot
        }
      }
      // while(!rslot_queue.empty()){
      //   Slot *temp = rslot_queue.top();
      //   rslot_queue.pop();
      //   temp->atomic.kv_len = 0; //清空原来的Slot
      // }
    ...
    //覆盖左右两组bucket
    nm_->rdma_write_sid_sync(0, r_laddr, rkey, (uint64_t)local_buf_, lkey,
                            sizeof(Bucket) * 4);
    // nm_->rdma_write_sid_sync(0, r_raddr, rkey, (uint64_t)local_buf_ + sizeof(Bucket) * 4, lkey,
    //                         sizeof(Bucket) * 4);
  }
  return 0;
}
